...
def preview():
    global rpistr,count,p, brightness,contrast,modes,mode,red,blue,gain,sspeed,ev,preview_width,preview_height,zoom,igw,igh,zx,zy,awbs,awb,saturations,saturation,meters,meter,flickers,flicker,sharpnesss,sharpness
    ...
    if speed2 > 6000000:
        speed2 = 6000000
    rpistr = "libcamera-still -n -e jpg -t 0 --width " + str(preview_width) + " --height " + str(preview_height) + " -o /run/shm/test%d.jpg --timelapse 30 "
    ...
    rpistr += " --denoise "    + denoises[denoise] # + " --framerate 5 "
...
while True:
    ...
    if len(pics) > 1:
        try:
            image = pygame.image.load(pics[len(pics)-1])
              ...
              if button_column == 2:                       
                if button_row == 1:
                   ...
                   if tinterval < 20:
                       ...
                       timestamp = now.strftime("%y%m%d%H%M%S")
                       if extns[extn] != 'raw':
                           rpistr = "libcamera-still -e " + extns[extn] + " -n -t " + str(tduration * 1000) + " --timelapse " + str(tinterval * 1000) + " -o /home/pi/Pictures/" + timestamp + "_%04d." + extns[extn]
                       else:
