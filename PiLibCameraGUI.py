...
# set default limits
mode        = 1       # set camera mode ['manual','auto'] 
speed       = 13      # position in shutters list
gain        = 0       #
brightness  = 0       # set camera brightness -100 to 100
contrast    = 70      # set camera contrast 0 to 200
ev          = 0       # eV correction -10 to 10
...
config_file = "/home/pi/PiLCConfig1.txt"
...
# data
modes        = ['manual','normal']
extns        = ['jpg','png','bmp','gif','rgb','yuv420']
...
still_limits = ['mode',0,len(modes)-1,'speed',0,len(shutters)-1,'gain',0,100,'brightness',-100,100,'contrast',0,200,'ev',-10,10,'blue',1,80,'red',1,80,'extn',0,len(extns)-1,'saturation',0,20,'meter',0,len(meters)-1,'awb',0,len(awbs)-1]
video_limits = ['vlen',1,999,'fps',2,40,'vformat',0,len(vwidths)-1,'0',0,0,'a_video',0,2,'zoom',0,4,'Focus',0,1,'tduration',1,9999,'tinterval',1,999,'tshots',1,999,'flicker',0,3,'codec',0,len(codecs)-1]

# check PiCconfigX.txt exists, if not then write default values
if not os.path.exists(config_file):
    points = [mode,speed,gain,brightness,contrast,frame,red,blue,ev,vlen,fps,vformat,codec,tinterval,tshots,tduration,extn,zx,zy,zoom,saturation,meter,awb,flicker,drc]
...
def preview():
    global count,p, brightness,contrast,modes,mode,red,blue,gain,sspeed,ev,preview_width,preview_height,zoom,igw,igh,zx,zy,awbs,awb,saturations,saturation,meters,meter,flickers,flicker,drcs,drc
    ...
    if speed2 > 6000000:
        speed2 = 6000000
    rpistr = "libcamera-still -e bmp -n -t 0 --width " + str(preview_width) + " --height " + str(preview_height) + " -o /run/shm/test%d.bmp --framestart 10  --timelapse 200 "
...
if os.path.exists('test.bmp'):
   os.rename('test.bmp', 'oldtest.bmp')
rpistr = "libcamera-jpeg -n -t 1000 -e bmp -o test.bmp"
os.system(rpistr)
time.sleep(2)
if os.path.exists('test.bmp'):
   imagefile = 'test.bmp'
...
while True:
    time.sleep(0.1)
    #print ('/run/shm/test' + str(count - 1) + '.bmp')
    if os.path.exists('/run/shm/test' + str(count - 1) + '.bmp'):
        try:
            image = pygame.image.load('/run/shm/test' + str(count - 1) + '.bmp')
        except pygame.error:
            pass
        os.rename('/run/shm/test' + str(count - 1 ) + '.bmp', '/run/shm/oldtest.bmp')
        count +=1
        ...
        if zoom > 0:
            ...
            if (vwidth == 1920 and vheight == 1080) or (vwidth == 1280 and vheight == 720):
            ...
            if vwidth == 1440 and vheight == 1080:
                pygame.draw.rect(windowSurfaceObj,(155,0,150),Rect(preview_width * 0.22,preview_height * 0.22,preview_width * 0.57,preview_height * 0.56),1)
        pygame.display.update()
    restart = 0
            ...
            if button_row == 2:
                ...
                for f in range(0,len(still_limits)-1,3):
                    if still_limits[f] == 'gain':
                        ...
                        pmax = still_limits[f+2]
                if mousey < ((button_row-1)*bh) + 10:
                    gain = int(((mousex-preview_width) / bw) * (pmax-pmin))
                ...
                for f in range(0,len(still_limits)-1,3):
                    if still_limits[f] == 'brightness':
                        ...
                        pmax = still_limits[f+2]
                if mousey < ((button_row-1)*bh) + 10:
                    brightness = int(((mousex-preview_width) / bw) * (pmax-pmin)) + pmin 
                elif mousey < ((button_row-1)*bh) + (bh/1.2):
                    if mousex > preview_width + (bw/2):
                        brightness +=1
                ...
                for f in range(0,len(still_limits)-1,3):
                    if still_limits[f] == 'contrast':
                        ...
                        pmax = still_limits[f+2]
                if mousey < ((button_row-1)*bh) + 10:
                    contrast = int(((mousex-preview_width) / bw) * (pmax-pmin)) + pmin
                elif mousey < ((button_row-1)*bh) + (bh/1.2):
                    if mousex > preview_width + (bw/2):
                        contrast +=1
                ...
                for f in range(0,len(still_limits)-1,3):
                    if still_limits[f] == 'ev':
                        ...
                        pmax = still_limits[f+2]
                if mousey < ((button_row-1)*bh) + 10:
                    ev = int(((mousex-preview_width) / bw) * (pmax-pmin)) + pmin
                elif mousey < ((button_row-1)*bh) + (bh/1.2):
                    if mousex > preview_width + (bw/2):
                        ev +=1
                        ev = min(ev,pmax)
            ...
            if button_row == 2:
                ...
                for f in range(0,len(video_limits)-1,3):
                    if video_limits[f] == 'codec':
                        pmin = video_limits[f+1]
                        pmax = video_limits[f+2]
                if mousey < ((button_row-1)*bh) + (bh/1.2):
                  if mousex > preview_width + bw + (bw/2):
                      codec +=1
            ...
            if mousex > preview_width:
              ...
              if button_column == 2:                       
                if button_row == 1:
                   ...
                   now = datetime.datetime.now()
                   timestamp = now.strftime("%y%m%d%H%M%S")
                   vname =  vid_dir + str(timestamp) + "." + codecs[codec]
                   rpistr = "libcamera-vid --codec mjpeg -t " + str(vlen * 1000) + " -o " + vname + " --framerate " + str(fps)
                   rpistr += " --brightness " + str(brightness/100) + " --contrast " + str(contrast/100)
                   if zoom > 0:
                       rpistr += " --width " + str(preview_width) + " --height " + str(preview_height)
                   else:
                   ...
                   if tinterval < 20:
                     ...
                     while tcount < tshots:
                       ...
                       now = datetime.datetime.now()
                       timestamp = now.strftime("%y%m%d%H%M%S")
                       fname =  pic_dir + str(timestamp) + '_' + str(tcount) + '.' + extns[extn]
                       rpistr = "libcamera-still -e " + extns[extn] + " -n -t 1000 -o " + fname 
                       rpistr += " --brightness " + str(brightness/100) + " --contrast " + str(contrast/100)
                       if zoom > 0:
                           rpistr += " --width " + str(preview_width) + " --height " + str(preview_height) 
