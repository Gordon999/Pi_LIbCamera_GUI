...
config_file = "/home/pi/PiLCConfig2.txt"
...
extns        = ['jpg','png','bmp','gif','rgb','yuv420','raw']
...
video_limits = ['vlen',1,999,'fps',2,40,'vformat',0,len(vwidths)-1,'0',0,0,'sharpness',0,4,'denoise',0,len(denoises)-1,'zoom',0,4,'Focus',0,1,'tduration',1,9999,'tinterval',1,999,'tshots',1,999,'flicker',0,3,'codec',0,len(codecs)-1]

# check PiCconfigX.txt exists, if not then write default values
if not os.path.exists(config_file):
    points = [mode,speed,gain,brightness,contrast,frame,red,blue,ev,vlen,fps,vformat,codec,tinterval,tshots,tduration,extn,zx,zy,zoom,saturation,meter,awb,flicker,sharpness,denoise]
...
def preview():
    global count,p, brightness,contrast,modes,mode,red,blue,gain,sspeed,ev,preview_width,preview_height,zoom,igw,igh,zx,zy,awbs,awb,saturations,saturation,meters,meter,flickers,flicker,sharpnesss,sharpness
    ...
    if speed2 > 6000000:
        speed2 = 6000000
    rpistr = "libcamera-still -e bmp -n -t 0 --width " + str(preview_width) + " --height " + str(preview_height) + " -o /run/shm/test%d.bmp --framestart 10  --timelapse 100 "
...
while True:
    ...
    pics = glob.glob('/run/shm/*.bmp')
    if os.path.exists('/run/shm/test' + str(count - 1) + '.bmp') and len(pics) > 2:
        try:
